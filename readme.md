...
* `$_ModelInstance` which is always equal to `true`, and is used internally when setting a DeriveJS object value to an instance of another DeriveJS object (in which case it will be saved as a DBRef object).
* [`$DefaultCriteria`](#defaultcriteria) (which is explained later). 
* [`$_BARE`](#$bare) which contains the "raw" (unproxified) document. Setting the values of the $_BARE object - will **not** affect the database.
* [`$_dbEvents`](#database-persistence-events-_dbevents) - an EventEmitter for listening and responding to DB persistence events.
and
* [`$Listen`](#listen), which is not created automatically but can be defined as an array of property names that you want their value-changes to be "listened" to (as explained in "[Listening for changes](#listening-for-local-changes)"). The 4th one is 
